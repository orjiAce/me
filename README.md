...
- [x] **M10 Hardening** — Playwright E2E against the production build (webpack — the split-bundler divergence check; found the `dynamicParams=false` prod-404 divergence and fixed it), 35 functional tests + 13 axe-clean routes (WCAG A/AA, zero violations), 21 visual baselines at 375/768/1440 (reduced-motion, npm counts masked), §17 rows 19/22/23/25/28/32 + 200% zoom exercised for the first time; Lighthouse: `/` 93·100·96·100, `/work` 94·100·96·91†. Fixes shipped: in-text links underlined (WCAG 1.4.1), heading levels configurable on the spine (§15 no-skip), wordmark accessible-name mismatch, print CSS scoped to site chrome, 320px scroll budget to <12 screens, `confidential` rendering path (row 8)

† `/work` SEO 91 is a Lighthouse artifact: Next streams metadata for dynamic
routes and the audit only reads the head — the description tag is verified
present in the served HTML. Open finding: LCP 3.2s (simulated) vs the §14
2.0s target — the perf score meets its ≥92 budget; hero-font LCP tuning is
the remaining lever.
